...
engine = create_engine(
    url=f"postgresql+psucopg2://{BD_USER}:{BD_PASSWORD}:@{BD_HOST}:{BD_NAME}",
    echo=True,
    # pool_size=5,
    # max_overflow=10,
)
...
def postmessage(messagedata : Message, bt : BackgroundTasks):
    bt.add_task(main, messagedata.chat_id, messagedata.user_id, messagedata.message)
    return {"status": "sent", "received": messagedata.message}

def main(id, chat_id, message):
    params = [str(id), str(chat_id), str(message)]
